...
leng = len(test_df['text'].values)
for i in range(leng):
  if (type(test_df['text'].values[i]) == type(0.5)):
    test_df['text'].values[i] = 'ảnh'
 
# test_df['text'] = clean_text_test(test_df['text'])
...
my_submission = pd.DataFrame({'RevId': np.array(test_df["id"]), 'Rating': np.array(test_df["Rating"])})
